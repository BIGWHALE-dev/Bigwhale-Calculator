...
import useCalculatorLogic from "./controller/logicHandler";
...
function Keypad({ currDisplay, writeOnScreen, setAnswer }) {
  const btnClickHandler = useCalculatorLogic({
    currDisplay,
    writeOnScreen,
    setAnswer,
  });
    ...
    <div className="keypad">
      {keyTexts.map((key, index) => (
        <Btn
          ...
          onclick={() => btnClickHandler(key.type, key.text)}
        >
          {key.text}
        </Btn>
      ))}
